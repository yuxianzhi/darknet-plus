...
layer make_normalization_layer(int batch, int w, int h, int c, int size, float alpha, float beta, float kappa)
{
    ...
    layer.output_gpu =  hip_make_array(layer.output, h * w * c * batch);
    layer.delta_gpu =   hip_make_array(layer.delta, h * w * c * batch);
    layer.squared_gpu = hip_make_array(layer.squared, h * w * c * batch);
    layer.norms_gpu =   hip_make_array(layer.norms, h * w * c * batch);
    #endif
    return layer;
}
...
#ifdef GPU
    hip_free(layer->output_gpu);
    hip_free(layer->delta_gpu); 
    hip_free(layer->squared_gpu); 
    hip_free(layer->norms_gpu);   
    layer->output_gpu =  hip_make_array(layer->output, h * w * c * batch);
    layer->delta_gpu =   hip_make_array(layer->delta, h * w * c * batch);
    layer->squared_gpu = hip_make_array(layer->squared, h * w * c * batch);
    layer->norms_gpu =   hip_make_array(layer->norms, h * w * c * batch);
